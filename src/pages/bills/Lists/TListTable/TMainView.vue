  ...
  <TRouterPanel :imt>
    ...
    <div class="p2">
      ...
      <div class="flex items-center gap-2 mb-3">
        <Button v-for="tab in FILTER_TABS" :key="tab.key" :severity="activeFilter === tab.key ? 'primary' : 'secondary'" rounded text size="small" @click="activeFilter = tab.key">
          {{ tab.label }}
...
const FILTER_TAG_MAP: Record<string, string[]> = {
  all: [], vip: ['VIP'], new: ['新客'], active: ['老客', '意向'], lost: ['流失']
};

const filteredList = computed(() => {
  let result = customers.value;
  const filterTags = FILTER_TAG_MAP[activeFilter.value];
  if (filterTags?.length) result = result.filter((item) => filterTags.includes(item.tag));
  if (searchQuery.value.trim()) {
    const q = searchQuery.value.trim().toLowerCase();
    result = result.filter((item) => item.name.toLowerCase().includes(q) || item.phone.toLowerCase().includes(q) || item.id.toLowerCase().includes(q));
  }
  return result;
});
