  ...
  <TRouterPanel>
    ...
    <div class="p2">
      <div class="grid grid-cols-4 gap-4 mb-4">
        <div v-for="(stat, index) in PRODUCT_STATS" :key="index" class="card p-3">
      ...
      <DataTable :value="productData" stripedRows paginator :rows="10" :rowsPerPageOptions="[5, 10, 20]" dataKey="id" class="w-full">
        <Column field="name" header="产品名称" />
        <Column field="sales" header="销量">
          <template #body="slotProps">{{ formatCurrency(slotProps.data.sales) }}</template>
        </Column>
        <Column field="stock" header="库存" />
        <Column field="profit" header="利润">
          <template #body="slotProps"><span class="text-green-600 font-medium">{{ formatCurrency(slotProps.data.profit) }}</span></template>
        </Column>
